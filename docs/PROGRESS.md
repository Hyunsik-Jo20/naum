...
### 최근 추가(2026-06-17) — 방문 삭제 + 일일 초기화
- **방문 삭제**: 대기 카드(✕)·처치 화면(삭제 버튼)에서 학생이 교실로 가버린 경우 제거. `visits.deleteVisit`(로컬+클라우드+offline 큐), `supabaseBackend.deleteVisit`(visits+visit_links), **`0007_visit_delete.sql`(authenticated delete 정책) 실행 필요**.
- **하루 지나면 빈 화면**: NurseQueue가 대기/처치/완료를 **오늘(createdAt>=자정) 건만** 표시 → 어제 데이터 자동 제외(기록은 클라우드·교장보고 보존). 20초 틱으로 자정 롤오버 반영.
...
- ✅ **공식 기상특보·지진**(기상청 getWthrWrnList·getEqkMsg) — 승인·**연동 완료(2026-07-10)**. `fetchOfficialAlerts()`가 부산 특보(stnId=159)·지진(최근 3일)을 파싱해 파생 경보와 병합(`useOfficialAlerts` 훅). 특보 자유문을 유형·등급·동작으로 분해, 발효 중(해제 제외)만 노출.
- ⏳ **행안부 긴급재난문자** — 미승인(IP 등록 필요 / safetydata 별도 가능성). 승인 시 `fetchOfficialAlerts`에 소스 추가.
