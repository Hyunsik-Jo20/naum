...
**③ 명부·계정**
- **성별 전원 '남' 인식 수정**: 머리글 정규화("성 별"·"성별(남/여)"·"남/여") + 값 확대(남자/여자·M/F·1/2) + 머리글 없으면 값 90%로 열 자동 탐지. 미리보기에 남·여 인원 표시(여 0명이면 경고). **기존 업로드분은 재업로드 필요**.
...
| 조치 | 내용 | 미조치 시 |
|---|---|---|
| **학생 명부 재업로드** | 성별 파싱 수정 전 올린 명부는 전원 "남"으로 굳어 있다. 같은 파일을 다시 올리면 정정됨(미리보기에 남·여 인원 표시). **성별이 틀리면 혈압 참고범위도 틀어진다**(3학년부터 남녀 상한이 다름) | 여학생이 전부 남으로 집계·표시 + 혈압 경고 오작동 |
