...
> 최종 업데이트: 2026-07-15 (담임교사 계정·교사→보건교사 요청·인체도·보건일지 증상/결과·공식경보 정확도)
...
| 조치 | 무엇 | 안 하면 |
|---|---|---|
| **마이그레이션 0011** 적용 | 담임교사 계정이 담당 학년/반을 잡도록(트리거가 app_metadata grade/classNo 기록) | 담임 가입은 되나 교사 화면이 반을 못 잡음 |
| **마이그레이션 0009** 적용(스테이징 먼저) | RLS 강화(visits·app_state 쓰기를 nurse/edu로) | 보안만 미적용, 앱은 정상 |
...
**적용 완료(참고)**: 0008(토큰 게이트)·0010(교사→보건교사 relay)·키서버 Phase 1·토큰 게이트 env. **기상특보·지진은 실연동 완료.**
...
| 0011_profile_class_from_app_meta | 가입 트리거가 app_metadata의 grade·classNo도 profiles에 기록(담임교사 계정) | ⏳ **미적용** — 담임교사 계정 로그인이 반을 잡으려면 필요 |
