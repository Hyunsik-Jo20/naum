...
- **공식 기상특보·지진 연동**(2026-07-10): data.go.kr 승인 완료 → `disasters.fetchOfficialAlerts`가 기상청 **기상특보**(`/api/kmawrn/getWthrWrnList`, 부산 stnId=159)·**지진**(`/api/kmaeqk/getEqkMsg`, 최근 3일)을 조회·파싱. 특보 title 자유문을 유형·등급·동작(발표/해제/변경)으로 분해, 최근 3일 이력을 접어 **현재 발효분(해제 제외)만** 노출. `useOfficialAlerts` 공유 훅(10분 캐시)으로 콘솔·상단바·교육청이 파생 경보와 병합 표시. 실검증: 부산 폭염경보/폭염·풍랑·열대야 주의보 실데이터 렌더. 남은 미승인=행안부 긴급재난문자(IP 등록 필요).
