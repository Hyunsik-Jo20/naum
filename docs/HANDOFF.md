...
| 조치 | 무엇 | 안 하면 |
|---|---|---|
| **키서버 Phase 2** | **전제 검증 완료(2026-07-29): 서버키==로컬키 바이트 동일 실증 → 제거 안전**. 남은 건 **Vercel에서 `VITE_SCHOOL_LINK_SECRET` 삭제(Prod·Preview) + 캐시 없이 재배포**뿐. 검증 명령·주의(레거시 토큰 재발급)는 [SUPABASE_SETUP §5-2]. | 마스터 비밀이 번들에 남음(현재 `index-*.js`에 존재 확인) |
