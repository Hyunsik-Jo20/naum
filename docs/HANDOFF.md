...
> 최종 업데이트: 2026-09-22 (**현장 운영 피드백 대량 반영 + 로컬 데이터 유실 버그 수정 + 활력징후**. 멀티테넌트 라이브 운영 중 — 실학교 운영 사용)
...
| 조치 | 내용 | 미조치 시 |
|---|---|---|
| **학생 명부 재업로드** | 성별 파싱 수정 전 올린 명부는 전원 "남"으로 굳어 있다. 같은 파일을 다시 올리면 정정됨(미리보기에 남·여 인원 표시) | 여학생이 전부 남으로 집계·표시 |
| **활력징후 참고범위 확인** | 기본값은 학령기 일반 기준 — 처치화면 활력징후 → "항목 편집"에서 학교급에 맞게 조정 | 정상인데 빨간 경고가 뜨거나, 이상값을 놓침 |
