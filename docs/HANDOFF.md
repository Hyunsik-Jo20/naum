...
- **⏳ 사용자 조치(순서)**: ① `0012_multi_tenant_scope.sql` 적용(비파괴) ② push 배포 후 **정리 SQL**(visit_links·relay_* truncate — 키 체계 변경으로 기존 암호문 무효, 초기화 확정됨) ③ 기존 교사/학부모 토큰 재발급. 상세: [SUPABASE_SETUP §5-3].
