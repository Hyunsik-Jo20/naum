...
import { callAi, loadAiConfig, isConfigured } from './ai'
...
/** 증상 라벨 배열 → 추천 결과. AI 미설정 시 예외. */
export async function aiTriage(symptoms: string[], grade: number, sex: string): Promise<AiTriageResult> {
  const cfg = loadAiConfig()
  if (!isConfigured(cfg)) throw new Error('AI 설정(제공자·API 키·모델)이 필요합니다. "AI 설정"에서 키를 입력하세요.')

  const system =
    '당신은 학교 보건실 보건교사를 돕는 임상 보조입니다. 학생이 키오스크에서 고른 증상을 보고 ' +
    '가능성 높은 병명과 계통, 감염병 의심 여부, 보건실에서 바로 할 수 있는 기본 처치를 제안합니다. ' +
